...
df = pd.DataFrame({'Fold':fold,'Accuracy per Fold':accuracy_per_fold,'Precision per Fold':precision_per_fold,'Recall per Fold':recall_per_fold,'F-Measure per Fold':fmeasure_per_fold,'Accuracy':accuracy_total_accumulation})
print(df)

df.to_excel(r'outputneweditedkfoldtala.xlsx', index = False, header=True)
