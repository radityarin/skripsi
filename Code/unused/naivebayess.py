...
class NBMultinomial(object):
    ...
    def predict(self,data_test,expected_result):
        ...
        netral = 1
        positif = 1
        pengaruh_terbesar_term = ""
        pengaruh_terbesar_angka = 0
        list_perkalian_neg = []
        list_perkalian_net = []
        list_perkalian_pos = []
        termnya = []
        for term in self.used_terms:
            ...
            list_perkalian_neg.append(term)
            list_perkalian_neg.append(self.used_terms_with_likelihood[term][0])
            list_perkalian_net.append(term)
            list_perkalian_net.append(self.used_terms_with_likelihood[term][1])
            list_perkalian_pos.append(term)
            list_perkalian_pos.append(self.used_terms_with_likelihood[term][2])

            if self.used_terms_with_likelihood[term][0] > pengaruh_terbesar_angka:
                pengaruh_terbesar_angka = self.used_terms_with_likelihood[term][0]
                pengaruh_terbesar_term = term + " negatif"
            if self.used_terms_with_likelihood[term][1] > pengaruh_terbesar_angka:
                pengaruh_terbesar_angka = self.used_terms_with_likelihood[term][1]
                pengaruh_terbesar_term = term + " netral"
            if self.used_terms_with_likelihood[term][2] > pengaruh_terbesar_angka:
                pengaruh_terbesar_angka = self.used_terms_with_likelihood[term][2]
                pengaruh_terbesar_term = term + " positif"
        ...
        return finalResult, negatif, netral, positif, pengaruh_terbesar_angka, pengaruh_terbesar_term,list_perkalian_neg,list_perkalian_net,list_perkalian_pos, termnya
