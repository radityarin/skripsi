...
df = pd.DataFrame({'X':x_array,'Y':y_array,'L':l_array,'K-Fold':kfold_per_combination,'AccNeg':acc_neg,'AccNet':acc_net,'AccPos':acc_pos,'PreNeg':prec_neg,'PreNet':prec_net,'PrePos':prec_pos,'RecNeg':recall_neg,'RecNet':recall_net,'RecPos':recall_pos,'FMeNeg':fmeasure_neg,'FMeNet':fmeasure_net,'FmePos':fmeasure_pos,'Accuracy':acc_per_fold,'Combination Acc':combination_accuracy})
print(df)
df.to_excel(r'pengujian1cobaulang3.xlsx', index = False, header=True)
